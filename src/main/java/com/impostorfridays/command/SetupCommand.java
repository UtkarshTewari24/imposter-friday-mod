...
public final class SetupCommand {
	...
	private static void applySetting(ServerCommandSource source, String key, String value) {
		...
		try {
			switch (key) {
				...
				case "resetadv" -> cfg.setResetAdvancementsOnStart(Boolean.parseBoolean(value));
				default -> {
	...
	private static void showPanel(ServerCommandSource source) {
		...
		send(source, toggleRow("Reset advancements at /start", cfg.isResetAdvancementsOnStart(),
				"resetadv"));
